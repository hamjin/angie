...
void
ngx_xquic_intercom_send(xqc_engine_t *engine, ngx_int_t worker_num, ngx_xquic_recv_packet_t *packet)
{
    ngx_int_t           n;
    ngx_err_t           err;
    ngx_connection_t   *c;
    ...
    n = sendto(c->fd, packet, sizeof(ngx_xquic_recv_packet_t), 0,
               (const struct sockaddr *)&ctx->addr[worker_num], ctx->addrlen[worker_num]);
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_EVENT, ctx->log, 0,
                   ...
                   ngx_worker, worker_num, xqc_dcid_str(engine, &packet->xquic.dcid),

...
my $t = Test::Nginx->new()->has(qw/http http_ssl http_v2 http_spdy proxy/)
	->skip_api_check()
	->has_daemon('openssl');
...
SKIP: {
skip 'no ALPN support in OpenSSL', 3
	if $t->has_module('OpenSSL') and not $t->has_feature('openssl:1.0.2');

$r = spdy_suite($t, version => '3', mode => 'alpn');
ok($r->{ok}, 'SPDY/3 ALPN request suite')
	or diag($r->{output});
...
$r = spdy_client($t, version => '3.1', mode => 'alpn',
	alpn => 'http/1.1,spdy/3.1');
ok($r->{ok}, 'SPDY/3.1 ALPN client order http before spdy')
	or diag($r->{output});

/*
 * Standalone SPDY/3.1 TLS GET client.
 *
 * Build:
 *   cc -Wall -Wextra -O2 /tmp/spdy31_client.c -o /tmp/spdy31_client \
 *      $(pkg-config --cflags --libs openssl zlib)
 *
 * Usage:
 *   /tmp/spdy31_client host [port] [marker] [alpn|npn|both]
 *   SPDY_EXPECT=marker /tmp/spdy31_client host [port]
 *   SPDY_PATH=/large /tmp/spdy31_client host [port] [marker] [alpn|npn|both]
 *   SPDY_SUITE=1 /tmp/spdy31_client host [port] [marker] [alpn|npn|both]
 */
...
struct response {
    ...
    unsigned received_stream_window_updates;
...
static unsigned char *parse_proto_list(const char *value, size_t *len)
{
    const char *p, *q;
    ...
    for ( ;; ) {
        ...
        if (n == 0 || n > 255) {
            die("invalid SPDY_ALPN protocol list");
        }
    ...
    const struct request_spec *req)
{
    ...
    add_nv(&raw, "user-agent", "spdy31-client/1");
...
static void read_response(SSL *ssl, struct response *r)
{
    ...
    uint16_t type;
    int control, done, stream_recv_window, connection_recv_window;
    unsigned char flags;
    z_stream z;
    ...
    connection_recv_window = INITIAL_WINDOW_SIZE;
        ...
        if (!control) {
            ...
            if (sid == 1) {
                if (len > (uint32_t) stream_recv_window) {
                    die("server exceeded SPDY/3.1 stream receive window");
                }

                if (len > (uint32_t) connection_recv_window) {
                    die("server exceeded SPDY/3.1 connection receive window");
                }

                stream_recv_window -= (int) len;
                connection_recv_window -= (int) len;
                append_bytes(&r->body, payload, len);
                fwrite(payload, 1, len, stdout);
                send_window_update(ssl, 0, len);
                r->sent_connection_window_updates++;
            ...
            case TYPE_WINDOW_UPDATE:
                ...
                if (sid == 0) {
                    ...
                    r->received_stream_window_updates++;
                }
                break;
...
static void check_response(const struct request_spec *req, struct response *r)
{
    ...
    if (r->received_connection_window_updates == 0) {
        fprintf(stderr,
            "\nerror: SPDY/3.1 did not receive connection WINDOW_UPDATE\n");
        exit(1);
    }

    if (r->body.n > INITIAL_WINDOW_SIZE && r->sent_stream_window_updates == 0) {
        fprintf(stderr, "\nerror: SPDY/3.1 did not send stream WINDOW_UPDATE\n");
        exit(1);
    }

    if (r->body.n > INITIAL_WINDOW_SIZE
        && r->sent_connection_window_updates == 0)
    {
        fprintf(stderr,
            "\nerror: SPDY/3.1 did not send connection WINDOW_UPDATE\n");
        exit(1);
    }
}
...
{
    static const unsigned char proto[] = "\010spdy/3.1";
    (void) ssl;
    (void) arg;
...
static void check_negotiated(SSL *ssl, const char *mode)
{
    const unsigned char *p = NULL;
    unsigned int n = 0;

    if (strcmp(mode, "npn") != 0) {
        SSL_get0_alpn_selected(ssl, &p, &n);
        if (n == 8 && memcmp(p, "spdy/3.1", 8) == 0) {
            return;
        }
    }
    ...
    die("TLS did not negotiate spdy/3.1");
}

static void run_request(SSL_CTX *ctx, const char *host, const char *port,
    const char *mode, const struct request_spec *req)
{
    SSL *ssl;
    int fd, rc;
    struct response r = {0};
    ...
    const char *mode)
{
    ...
    for (i = 0; suite[i].method != NULL; i++) {
        run_request(ctx, host, port, mode, &suite[i]);
    }
}

static void usage(const char *prog)
{
    fprintf(stderr, "usage: %s host [port] [expected-marker] [alpn|npn|both]\n",
        prog);
    fprintf(stderr, "       SPDY_EXPECT=marker %s host [port]\n", prog);
    fprintf(stderr,
        "       SPDY_PATH=/path %s host [port] [expected-marker] [mode]\n",
        prog);
    fprintf(stderr, "       SPDY_SUITE=1 %s host [port] [expected-marker] [mode]\n",
        prog);
}

int main(int argc, char **argv)
{
    const char *host, *port, *marker, *mode, *path, *method, *body;
    const char *header, *value, *suite;
    const char *alpn_env;
    SSL_CTX *ctx;
    struct request_spec req;
    static const unsigned char alpn[] = "\010spdy/3.1";
    unsigned char *alpn_data = NULL;
    size_t alpn_len = sizeof(alpn) - 1;
    ...
    if (strcmp(mode, "alpn") != 0 && strcmp(mode, "npn") != 0
        && strcmp(mode, "both") != 0)
    {
        usage(argv[0]);
        return 2;
    ...
    if (strcmp(mode, "npn") != 0) {
        alpn_data = parse_proto_list(alpn_env, &alpn_len);

        if (SSL_CTX_set_alpn_protos(ctx,
                                    alpn_data != NULL ? alpn_data : alpn,
    ...
    if (suite != NULL && suite[0] != '\0' && strcmp(suite, "0") != 0) {
        ...
        run_request(ctx, host, port, mode, &req);
    }

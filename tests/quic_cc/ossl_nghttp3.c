    ...
    void *user_data)
{
    ...
    if (!SSL_set_incoming_stream_policy(conn->qconn,
                                        SSL_INCOMING_STREAM_POLICY_ACCEPT, 0)) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
            "failed to configure incoming stream policy");
        goto err;
    }
    ...
    void *user_data)
{
    ...
    /* Set the hostname we would validate the certificate against if enabled. */
    if (X509_VERIFY_PARAM_set1_host(SSL_get0_param(qconn), bare_hostname, 0) <= 0)
        goto err;
...
int OSSL_DEMO_H3_CONN_handle_events(OSSL_DEMO_H3_CONN *conn)
{
    ...
    if (!SSL_handle_events(conn->qconn))
        return 0;

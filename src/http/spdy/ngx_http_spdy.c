...
static ngx_int_t ngx_http_spdy_construct_cookie_header(ngx_http_request_t *r);
static ngx_int_t ngx_http_spdy_construct_host_header(ngx_http_request_t *r);
static ngx_int_t ngx_http_spdy_validate_request_header(ngx_http_request_t *r);
...
static ngx_int_t ngx_http_spdy_process_request_body(ngx_http_request_t *r,
    u_char *pos, size_t size, ngx_uint_t last, ngx_uint_t flush);
static ngx_int_t ngx_http_spdy_filter_request_body(ngx_http_request_t *r);
static void ngx_http_spdy_read_client_request_body_handler(
...
ngx_http_spdy_init(ngx_event_t *rev)
{
    ...
    if (c->ssl) {
        ...
        if (len == sizeof(NGX_SPDY_PROTO_31) - 1
            && ngx_strncmp(data, NGX_SPDY_PROTO_31, len) == 0)
        {
            sc->version_minor = 1;

        } else if (len == sizeof(NGX_SPDY_PROTO_30) - 1
                   && ngx_strncmp(data, NGX_SPDY_PROTO_30, len) == 0)
        {
            sc->version_minor = 0;
        ...
        if (len == sizeof(NGX_SPDY_PROTO_30) - 1
            && ngx_strncmp(data, NGX_SPDY_PROTO_30, len) == 0)
        {
            sc->version_minor = 0;
        }
    ...
    u_char *end)
{
    ...
    if (sid) {
        ...
        ngx_log_error(NGX_LOG_INFO, sc->connection->log, 0,
                      "client sent WINDOW_UPDATE frame with sid 0 "
                      "for SPDY/3.0");
    ...
    u_char *end)
{
    ...
    if (stream->no_flow_control
        && stream->recv_window < NGX_SPDY_STREAM_WINDOW / 4)
    {
    ...
    u_char *end)
{
    ...
    if (size) {
        ...
        if (r->request_body) {
            rc = ngx_http_spdy_process_request_body(r, pos, size,
                         sc->length == 0 && (sc->flags & NGX_SPDY_FLAG_FIN),
                         0);
    ...
    if (sc->flags & NGX_SPDY_FLAG_FIN) {
        ...
        if (r->request_body && size == 0) {
            rc = ngx_http_spdy_process_request_body(r, NULL, 0, 1, 0);
...
ngx_http_spdy_handle_request_header(ngx_http_request_t *r)
{
    ...
    if (h->key.len == sizeof("cookie") - 1
        && ngx_strncmp(h->lowcase_key, "cookie", sizeof("cookie") - 1) == 0)
    {
        ngx_str_t    *val;
        ngx_array_t  *cookies;

        cookies = r->spdy_stream->cookies;
...
static ngx_int_t
ngx_http_spdy_construct_cookie_header(ngx_http_request_t *r)
{
    u_char                     *buf, *p, *end;
    size_t                      len;
    ngx_str_t                  *vals;
    ...
    h->hash = ngx_hash(ngx_hash(ngx_hash(ngx_hash(ngx_hash('c', 'o'), 'o'),
                                        'k'), 'i'), 'e');
...
static ngx_int_t
ngx_http_spdy_construct_host_header(ngx_http_request_t *r)
{
    ngx_table_elt_t            *h;
    ngx_http_header_t          *hh;
    ngx_http_core_main_conf_t  *cmcf;
...
static ngx_int_t
ngx_http_spdy_validate_request_header(ngx_http_request_t *r)
{
    ngx_str_t                  host;
    ngx_http_core_srv_conf_t  *cscf;

    if (r->headers_in.server.len == 0 && r->host_end == NULL) {
        ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
    ...
    if (r->host_end) {
        ...
        if (r->headers_in.host) {
            ...
            {
                ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
                              "client sent :host and Host headers "
                              "with different values");
                ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
                return NGX_ERROR;
            }

        } else if (ngx_http_spdy_construct_host_header(r) != NGX_OK) {
            return NGX_ERROR;
        }
    }
    ...
    } else if (!r->spdy_stream->in_closed) {
...
ngx_http_spdy_parse_scheme(ngx_http_request_t *r)
{
    ...
    r->schema.len = r->header_end - r->header_start;
    r->schema.data = r->header_start;
...
ngx_http_spdy_parse_host(ngx_http_request_t *r)
{
    ...
    r->host_start = r->header_start;
    r->host_end = r->header_end;
...
ngx_http_spdy_run_request(ngx_http_request_t *r)
{
    ...
    if (ngx_http_spdy_construct_cookie_header(r) != NGX_OK) {
        return;
    }

    if (ngx_http_spdy_validate_request_header(r) != NGX_OK) {
        return;
    }
...
ngx_http_spdy_read_request_body(ngx_http_request_t *r)
{
    ...
    if (r->request_body_no_buffering || rb->filter_need_buffering) {
        if (len < (off_t) NGX_SPDY_INIT_STREAM_WINDOW) {
            len = NGX_SPDY_INIT_STREAM_WINDOW;
        }

        if (len > NGX_SPDY_STREAM_WINDOW) {
            len = NGX_SPDY_STREAM_WINDOW;
        }
    }
    ...
    if (stream->in_closed) {
        ...
        if (buf) {
            rc = ngx_http_spdy_process_request_body(r, buf->pos,
                                                    buf->last - buf->pos,
                                                    1, 0);
        ...
        r->read_event_handler = ngx_http_spdy_read_client_request_body_handler;
    ...
    if (r->request_body_no_buffering || rb->filter_need_buffering) {
        size = (size_t) len - NGX_SPDY_INIT_STREAM_WINDOW;

    } else {
        stream->no_flow_control = 1;
        size = NGX_SPDY_STREAM_WINDOW - stream->recv_window;
    }

    if (size) {
        if (ngx_http_spdy_send_window_update(stream->connection,
                                             stream->id, size)
            == NGX_ERROR)
        {
            stream->skip_data = NGX_SPDY_DATA_INTERNAL_ERROR;
...
static ngx_int_t
ngx_http_spdy_process_request_body(ngx_http_request_t *r, u_char *pos,
    size_t size, ngx_uint_t last, ngx_uint_t flush)
{
    ...
    for ( ;; ) {
            ...
            if (rb->buf->last == rb->buf->end && size) {
                ...
                if (r->request_body_no_buffering) {
                    ngx_log_error(NGX_LOG_ALERT, fc->log, 0,
                                  "no space in spdy body buffer");

                    return NGX_HTTP_INTERNAL_SERVER_ERROR;
                }

                ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                               "spdy body update chains");

                rc = ngx_http_spdy_filter_request_body(r);
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                       "spdy request body rest %O", rb->rest);

        if (flush || (last && rb->rest == 0)) {
            rc = ngx_http_spdy_filter_request_body(r);
...
static ngx_int_t
ngx_http_spdy_filter_request_body(ngx_http_request_t *r)
{
    ngx_buf_t                 *b, *buf;
    ngx_int_t                  rc;
    ngx_chain_t               *cl;
    ...
    b->tag = (ngx_buf_tag_t) &ngx_http_spdy_filter_request_body;
...
update:
    ...
    ngx_chain_update_chains(r->pool, &rb->free, &rb->busy, &cl,
                            (ngx_buf_tag_t) &ngx_http_spdy_filter_request_body);
...
static void
ngx_http_spdy_read_client_request_body_handler(ngx_http_request_t *r)
{
    size_t                     window;
    ngx_buf_t                 *buf;
    ngx_int_t                  rc;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                   "spdy read client request body handler");
    ...
    rc = ngx_http_spdy_process_request_body(r, NULL, 0,
                                            r->spdy_stream->in_closed, 1);
    ...
    if (r->spdy_stream->no_flow_control) {
        return;
    }
...
ngx_int_t
ngx_http_spdy_read_unbuffered_request_body(ngx_http_request_t *r)
{
    size_t                     window;
    ngx_buf_t                 *buf;
    ngx_int_t                  rc;
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                   "spdy read unbuffered request body");

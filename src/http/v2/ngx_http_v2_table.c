    ...
    ngx_int_t *header_idx)
{
    ...
    *header_idx = hpack_get_static_index(h2c, key, key_len);
    ...
    if (h2c->hpack_enc.top == 0xffffffff) {
        ...
        ngx_memzero(&h2c->hpack_enc, sizeof(ngx_http_v2_hpack_enc_t));
    }
    ...
    table[i].index    = h2c->hpack_enc.top;
    table[i].pos      = h2c->hpack_enc.pos;
    table[i].klen     = key_len;
    table[i].vlen     = val_len;
    table[i].size     = size;
    table[i].next     = 0;
    ...
    if (idx > 0) {
        ...
        *pos = 128;
        ...
        if (header_idx == NGX_ERROR) { /* if key is not present */
            ...
            if (idx == NGX_ERROR) {
                *pos = 0;
                pos = ngx_http_v2_write_int(pos, ngx_http_v2_prefix(4),
                                            header_idx);
                ...
                *pos = 64;
                pos = ngx_http_v2_write_int(pos, ngx_http_v2_prefix(6),
                                            header_idx);

                ngx_log_debug1(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                              "http2 hpack encode: Literal Header Field with "
                              "Incremental Indexing - Indexed Name: %ud",
                              header_idx);
            }
        }
                        ...
                        uint8_t *key, size_t key_len)
{
    ...
    for (i = 0; i < HPACK_ENC_DYNAMIC_KEY_TBL_SZ; i++) {
        ...
        if (name->hash_val == key_hash
            && ngx_memcmp(h2c->hpack_enc.storage + name->pos, key, key_len)
               == 0)
        {
...
hpack_get_static_index(ngx_http_v2_connection_t *h2c, u_char *val, size_t len)
{
    ...
    static const struct {
        ...
        {13, "cache-control",               24},
        {13, "last-modified",               44},
        {14, "content-length",              28},
        {16, "content-encoding",            26},
        {16, "content-language",            27},
        {16, "content-location",            29},
        {16, "www-authenticate",            61},
        {17, "transfer-encoding",           57},
        {18, "proxy-authenticate",          48},
        {19, "content-disposition",         25},
        {25, "strict-transport-security",   56},
        {27, "access-control-allow-origin", 20},
        {99, "",                            99},
    }, *header;

    /* for a given length, where to start the search
       since minimal length is 3, the table has a -3 offset */
